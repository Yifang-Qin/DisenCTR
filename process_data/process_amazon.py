...
def to_df(file_path):
  with open(file_path, 'r') as fin:
    ...
    return df.loc[:, ['reviewerID', 'asin', 'unixReviewTime']]
...
def gen_graph(user_count):
    ...
    for uid, hist in reviews_df.groupby('reviewerID'):
        item_list = hist['asin'].tolist()[: -1]
        time_list = hist['unixReviewTime'].tolist()[:-1]
...
if __name__ == '__main__':
    ...
    reviews_df = to_df(f'../raw_data/reviews_{dataset_type}_5.json')
    ...
    for reviewerID, hist in reviews_df.groupby('reviewerID'):
        pos_list = hist[['asin', 'unixReviewTime']].values
        def gen_neg():
            neg = pos_list[0, 0]
